...
def save_error(train_losses, valid_losses, epoch):
	train_losses = [train_losses[i]*error_big for i in range(len(train_losses))]
	valid_losses = [valid_losses[i]*error_big for i in range(len(valid_losses))]
	_, ax1 = plt.subplots()
	# ax2 = ax1.twinx()
...
def save_model(model, optimizer, scheduler, epoch, best_loss, train_losses, valid_losses, train_time, valid_time, path):
	info = {
...
def load_model(model, optimizer, scheduler, path):
	...
	return epoch, best_loss, train_losses, valid_losses, train_time, valid_time
	...
	for batch_idx, (data, target, mask) in enumerate(dataloader):
		...
		if (batch_idx+1) % log_rate == 0:
			...
			print('Train Epoch: {} [{}/{} ({:.0f}%)] \tLoss: {:.6f} \tTime: {:.0f}m {:.0f}s'.format(
				epoch, process, total_len, process / total_len * 100., avg_batch_loss*error_big, time_elapsed//60, time_elapsed%60))
...
if __name__ == '__main__':
	...
	if load is True:
		start_epoch, best_loss, train_losses, valid_losses, total_train_time, total_valid_time \
	...
	for epoch in range(start_epoch, num_epochs):
		...
		valid_loss, valid_time = valid_model(model, dataloaders['valid'], criterion, epoch)
		...
		if epoch % save_rate == 0:
			save_model(	model, optimizer, scheduler, epoch, best_loss, train_losses, valid_losses,
						total_train_time, total_valid_time, checkpoint_path + 'checkpoint_' + str(epoch) + '.tar')
			save_error(train_losses, valid_losses, epoch)
