...
class SketchToDepthModel(nn.Module):
	def __init__(self):
		...
		self.drelu6 = nn.ReLU()
		# 64 128x128
		self.dconv7 = nn.ConvTranspose2d(128, 6, 4, 2, 1)
		self.tanh = nn.Tanh()
		# 6 256x256
	...
	def forward(self, x):
		...
		y1 = F.dropout(self.drelu1(self.dbn1(self.dconv1(x7))))
		y2 = F.dropout(self.drelu2(self.dbn2(self.dconv2( torch.cat([x6, y1], dim=1) ))))
		...
		y7 =           self.dconv7( torch.cat([x1, y6], dim=1) )
		y = self.tanh(y7)
