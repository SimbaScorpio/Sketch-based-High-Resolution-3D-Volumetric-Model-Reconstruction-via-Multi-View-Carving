...
def save_depths(images, masks, epoch):
	# images *= (dim-1.0)
	images = (images+1.0)*((dim-1.0)/2.0)
	for i in range(6):
		img = render_depth(images[i], dim)
		a, b = np.where(masks[i] == 0)
		for x, y in zip(a, b):
...
def save_error(train_losses, valid_losses, base_losses, epoch):
	...
	base_losses = [base_losses[i]*error_big for i in range(len(base_losses))]
	_, ax1 = plt.subplots()
	# ax2 = ax1.twinx()
	...
	for batch_idx, (data, target, mask) in enumerate(dataloader):
		...
		if (batch_idx+1) % log_rate == 0:
			...
			print('Train Epoch: {} [{}/{} ({:.0f}%)] \tLoss: {:.6f} \tTime: {:.0f}m {:.0f}s'.format(
				epoch, process, total_len, process / total_len * 100., loss*error_big, time_elapsed//60, time_elapsed%60))
...
def valid_model(model, dataloader, criterion, epoch):
	...
	print('Valid Loss: {:.6f} \tL1 Loss: {:.6f} \tTime: {:.0f}m {:.0f}s'.format(
		avg_loss*error_big, avg_l1loss*error_big, time_elapsed//60, time_elapsed%60))
