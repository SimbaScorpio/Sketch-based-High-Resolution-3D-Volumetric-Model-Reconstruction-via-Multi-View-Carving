...
load = False
load_checkpoint = './checkpoint/chair256/best_checkpoint.tar'
...
def save_error(train_losses, valid_losses, valid_tp_acc, valid_iou, epoch):
	_, ax1 = plt.subplots()
	ax2 = ax1.twinx()
...
def save_model(model, optimizer, scheduler, epoch, best_loss, train_losses, valid_losses, valid_tp_acc, valid_iou, train_time, valid_time, path):
	info = {
...
def load_model(model, optimizer, scheduler, path):
	...
	return epoch, best_loss, train_losses, valid_losses, valid_tp_acc, valid_iou, train_time, valid_time
...
def train_model(model, dataloader, criterion, optimizer, epoch):
	...
	for batch_idx, (data, target) in enumerate(dataloader):
		...
		if (batch_idx+1) % log_rate == 0:
			...
			print('Train Epoch: {} [{}/{}] \tLoss: {:.6f} \tTime: {:.0f}m {:.0f}s'.format(
				epoch, process, total_len, loss, time_elapsed//60, time_elapsed%60))
...
def valid_model(model, dataloader, criterion, epoch):
	...
	with torch.no_grad():
		...
		for batch_idx, (data, target) in enumerate(dataloader):
			...
			un,_,_,_ = np.where((output>threshold)|(target>threshold))
			...
			iou.append( len(tp) / len(un) )

		avg_tp_acc = np.array(tp_acc).sum() / len(tp_acc)
		avg_iou = np.array(iou).sum() / len(iou)

		save_voxel(output[0], epoch)

	avg_loss = total_loss / len(dataloader)
	time_elapsed = time.time() - since
	print('Valid Loss: {:.6f} \tTPacc: {:.6f} \tIoU: {:.6f} \tTime: {:.0f}m {:.0f}s'.format(
		avg_loss, avg_tp_acc, avg_iou, time_elapsed//60, time_elapsed%60))
	return avg_loss, avg_tp_acc, avg_iou, time_elapsed
...
if __name__ == '__main__':
	...
	valid_tp_acc, valid_iou = [], []
	total_train_time = 0
	total_valid_time = 0
	start_epoch = 0

	if load is True:
		start_epoch, best_loss, train_losses, valid_losses, valid_tp_acc, valid_iou, total_train_time, total_valid_time \
	...
	for epoch in range(start_epoch, num_epochs):
		...
		valid_loss, tp_acc, iou, valid_time = valid_model(model, dataloaders['valid'], criterion, epoch)
		...
		if epoch % save_rate == 0:
			save_model(	model, optimizer, scheduler, epoch, best_loss, train_losses, valid_losses, valid_tp_acc, valid_iou,
						total_train_time, total_valid_time, checkpoint_path + 'checkpoint_' + str(epoch) + '.tar')
			save_error(train_losses, valid_losses, valid_tp_acc, valid_iou, epoch)
